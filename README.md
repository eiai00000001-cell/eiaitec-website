...
|---|---|---|---|---|---|
| ★C-1 | **作業中の誤操作の報告**: 画像配置作業の際、`frontend/app/favicon.ico`を誤ってユーザーの明示的許可なく削除した。直後に、同一バージョンのNext.jsで新規に`create-next-app`を実行して生成された既定favicon.ico(内容は元ファイルと同一のはずのビルド既定ファイル)を取得し、同じパスに復元した。 | 現状ファイルは復元済みで機能・見た目への影響はないと考えられるが、削除操作自体はルール違反であった。 | `git diff`やファイル内容の確認 | ユーザー本人 | 本報告確認時 |
| ★C-2 | モバイルナビ開閉ボタンの位置(6章D-6参照)。`mobile-nav.html`のキャプション文言と実際のHTML/画像が食い違っている。今回は画像・HTML(右側配置)を採用したが、意図どおりか確認したい。 | F-08の見た目 | 実装結果(本報告のスクリーンショット、または`npm run dev`起動後の実機確認)とキャプション文言を見比べて確認 | クライアント / デザイン担当 | 次回レビュー時 |
| ★C-3 | ローカル開発機のNode.jsバージョンがv25.2.1であり、基本設計書2.1節が指定するNode.js 20系(LTS)と異なる。今回の動作確認(`npm run dev`/`build`/`test`)はすべてv25.2.1で実施した。 | Vercel本番環境(F-11で設定)との差異により、将来的に挙動差が出る可能性(現時点でNode20系専用APIは使用していない)。 | F-11実装時にVercelプロジェクト設定(または`package.json`の`engines`フィールド)でNode.js 20系を明示的に指定し、あらためて動作確認する。 | developer(F-11実装時) | F-11着手時 |
...
| ★C-5 | Contactフォームの注記文言「※このフォームはモックアップです。実際の送信機能は本実装時に組み込みます。」は`mockup/index.html`の実際の表示文言をそのまま採用したが、F-07(実送信)実装後はこの文言が実態と合わなくなる。 | F-07実装時のUI文言 | F-07着手時に、この注記を残す/変更する/削除するかを確認する。 | クライアント | F-07着手時 |
| ★C-6 | 未使用の初期生成ファイルが残っている(削除の実行はしていません。ユーザーの明示的な許可後に削除します): `frontend/app/page.module.css`、`frontend/public/file.svg` `globe.svg` `next.svg` `vercel.svg` `window.svg`(いずれも`create-next-app`の既定テンプレートが生成した未使用ファイル)。 | 実害はないが、リポジトリの整理観点で削除候補。 | 削除してよいか確認のうえ、許可があればユーザー自身または次回作業時に削除する。 | ユーザー本人 | 任意 |
